...
'''Start game'''
def playGame(total_number_of_iterations, number_of_steps, iteration_gap):
    iterVal = 1
    iteration = 1
    global players
    ...
    while iteration <= total_number_of_iterations:
        ...
        if(acceptor in players[offeror].neighbors):
            ...
            #updateValuesAndLosses(acceptor, iterVal, offeror, sensitivity, number_of_steps)

            '''Check if success or failure'''
            checkConditionAndPerformActions(acceptor, iterVal, offeror, sensitivity)
            
            updateValuesAndLosses(acceptor, iterVal+1, offeror, sensitivity, number_of_steps)
            
            iterVal += 1
            iteration += 1
            
            if iterVal == iteration_gap:
...
def updateValuesAndLosses(acceptor, iterVal, offeror, sensitivity, number_of_steps):

    cur_offeror_val = players[offeror].offeror_value[iterVal]
    
    distance_to_above = (math.ceil(cur_offeror_val * number_of_steps) / number_of_steps) - cur_offeror_val
    distance_to_below = cur_offeror_val - (math.floor(cur_offeror_val * number_of_steps) / number_of_steps)
    
    probability_round_up = distance_to_below*number_of_steps
    probability_round_down = distance_to_above*number_of_steps
    
    randnum = random()
    
    if randnum <= probability_round_up:
        players[offeror].offeror_value[iterVal] = math.ceil(cur_offeror_val * number_of_steps) / number_of_steps
    else:
        players[offeror].offeror_value[iterVal] = math.floor(cur_offeror_val * number_of_steps) / number_of_steps
    ...
    distance_to_above = (math.ceil(cur_acceptor_val * number_of_steps) / number_of_steps) - cur_acceptor_val
    distance_to_below = cur_acceptor_val - (math.floor(cur_acceptor_val * number_of_steps) / number_of_steps)
    ...
    if randnum <= probability_round_up:
        players[acceptor].acceptor_value[iterVal] = math.ceil(cur_acceptor_val * number_of_steps) / number_of_steps
    else:
        players[acceptor].acceptor_value[iterVal] = math.floor(cur_acceptor_val * number_of_steps) / number_of_steps
...
def refreshPlayersAndStoreSteps(players):
    ...
    for player in players:
        ...
        del player.offeror_value[:]
        del player.acceptor_value[:]
...
def main():
    ...
    S = int(input("Enter the number of steps you'd like: "))
    T = int(input("Enter the number of iterations between recording: "))
    for iter in range(5 ):
        players = getNodes(S)
        finalPlayers = playGame(N,S,T)
